...
static int stmvl53l1_parse_tree(struct device *dev, struct i2c_data *i2c_data)
{
    ...
    if (IS_ERR_OR_NULL(i2c_data->pinctrl)) {
        vl53l1_wanrmsg("Failed to get pinctrl, please check dts");
        rc = PTR_ERR(i2c_data->pinctrl);
        goto err_pinctrl_get;
    }

    i2c_data->pins_active = pinctrl_lookup_state(i2c_data->pinctrl, "stmvl53l1_active");
    if (IS_ERR_OR_NULL(i2c_data->pins_active)) {
        vl53l1_wanrmsg("Pin state[active] not found");
        rc = PTR_ERR(i2c_data->pins_active);
        goto err_pinctrl_lookup;
    }

    i2c_data->pins_suspend = pinctrl_lookup_state(i2c_data->pinctrl, "stmvl53l1_suspend");
    if (IS_ERR_OR_NULL(i2c_data->pins_suspend)) {
        vl53l1_wanrmsg("Pin state[suspend] not found");
        rc = PTR_ERR(i2c_data->pins_suspend);
        goto err_pinctrl_lookup;
    }

    i2c_data->pins_release = pinctrl_lookup_state(i2c_data->pinctrl, "stmvl53l1_release");
    if (IS_ERR_OR_NULL(i2c_data->pins_release)) {
...
err_pinctrl_get:
    i2c_data->pinctrl = NULL;
    i2c_data->pins_release = NULL;
    i2c_data->pins_suspend = NULL;
    i2c_data->pins_active = NULL;
    return rc;
