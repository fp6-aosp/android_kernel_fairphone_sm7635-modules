...
static char *malloc_proc_buffer(u32 size)
{
    if (size > HUGE_SIZE) {
        malloc_type = 1;
...
static void free_proc_buffer(char **ptr)
{
    if (*ptr == NULL)
        return;
...
static int malloc_test_resource(void)
{
    ...
    if (raw_data_cnt > 0) {
        ...
        ts_test->deltadata = kcalloc(raw_data_cnt,
                sizeof(struct ts_test_rawdata), GFP_KERNEL);
        ts_test->stylusraw = kcalloc(raw_data_cnt,
                sizeof(struct ts_test_stylus_rawdata), GFP_KERNEL);
    }
    if (noise_data_cnt > 0) {
...
static u32 map_die2pin(u32 chn_num)
{
	...
	if (chn_num & DRV_CHANNEL_FLAG)
		chn_num = (chn_num & ~DRV_CHANNEL_FLAG) + ts_test->params_info->max_sen_num;
	...
	for (i = 0; i < short_ch_num; i++) {
		...
		for (j = short_die_num + 1; j < max_drv_num; j++) {
			...
			if (short_r < r_threshold) {
				...
				goodix_save_short_res(master_pin_num,
...
static int goodix_shortcircut_analysis(struct goodix_ts_core *cd)
{
    ...
    ret = cd->hw_ops->read(cd, ts_test->params_info->short_test_result_reg, temp_buf, sizeof(temp_buf));
    if (ret < 0) {
	...
	if (checksum_cmp(&temp_buf[sizeof(test_result)], checksum_len, CHECKSUM_MODE_U8_LE)) {
...
static int goodix_shortcircut_test(struct goodix_ts_core *cd)
{
    ...
    msleep(500); // berlinB have quick check, need wait 500ms

	/* get short test time */
    ret = cd->hw_ops->read(cd, ts_test->params_info->short_test_time_reg, (u8 *)&test_time, 2);
    if (ret < 0) {
    ...
    retry = 50;
	while (retry--) {
		ret = cd->hw_ops->read(cd, ts_test->params_info->short_test_status_reg, &status, 1);
...
static void goodix_save_header(struct goodix_ts_core *cd)
{
	...
	index = sprintf(&rbuf[index], "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
...
// 			if (chn1 == CHN_VDD)
// 				index += sprintf(&rbuf[index], "<ShortMess Chn1=\"VDD\" ");
// 			else if (chn1 == CHN_GND)
// 				index += sprintf(&rbuf[index], "<ShortMess Chn1=\"GND\" ");
// 			else if (chn1 & DRV_CHANNEL_FLAG)
// 				index += sprintf(&rbuf[index], "<ShortMess Chn1=\"Tx%d\" ",
// 						chn1 & 0x7f);
// 			else
// 				index += sprintf(&rbuf[index], "<ShortMess Chn1=\"Rx%d\" ",
// 						chn1 & 0x7f);
// 			if (chn2 == CHN_VDD)
// 				index += sprintf(&rbuf[index],
...
//     /* save open limits */
//     if (ts_test->item[GTP_CAP_TEST]) {
//         index += sprintf(&rbuf[index], "<Item name=\"Rawdata Test Sets\">\n");
//         index += sprintf(&rbuf[index], "<TotalFrameCnt>%d</TotalFrameCnt>\n",
//                 raw_data_cnt);
//         /* rawdata max limit */
//         index += sprintf(&rbuf[index], "<MaxRawLimit>\n");
//         for (i = 0; i < tx * rx; i++) {
//             index += sprintf(&rbuf[index], "%d,", ts_test->max_limits[i]);
//             if ((i + 1) % tx == 0)
//                 index += sprintf(&rbuf[index], "\n");
//         }
//         index += sprintf(&rbuf[index], "</MaxRawLimit>\n");
...
//         for (i = 0; i < tx * rx; i++) {
//             index += sprintf(&rbuf[index], "%d,", ts_test->min_limits[i]);
//             if ((i + 1) % tx == 0)
//                 index += sprintf(&rbuf[index], "\n");
//         }
//         index += sprintf(&rbuf[index], "</MinRawLimit>\n");
...
static void goodix_save_data(struct goodix_ts_core *cd)
{
    ...
    if (ts_test->item[GTP_CAP_TEST]) {
        ...
        for (i = 0; i < raw_data_cnt; i++) {
            goodix_data_cal(ts_test->rawdata[i].data, tx * rx, stat_result);
            index += sprintf(&rbuf[index],
                "<DataContent No.=\"%d\" DataCount=\"%d\" Maximum=\"%d\" Minimum=\"%d\" Average=\"%d\">\n",
                i, tx * rx, stat_result[1], stat_result[2], stat_result[0]);
            for (j = 0; j < tx * rx; j++) {
                index += sprintf(&rbuf[index], "%5d,", ts_test->rawdata[i].data[j]);
                if ((j + 1) % tx == 0)
                    index += sprintf(&rbuf[index], "\n");
            }
            index += sprintf(&rbuf[index], "</DataContent>\n");
            goodix_data_cal(ts_test->deltadata[i].data, tx * rx, stat_result);
            index += sprintf(&rbuf[index],
    ...
    if (ts_test->item[GTP_NOISE_TEST]) {
        ...
        for (i = 0; i < noise_data_cnt; i++) {
            goodix_data_cal(ts_test->noisedata[i].data, tx * rx, stat_result);
            index += sprintf(&rbuf[index],
                "<DataContent No.=\"%d\" DataCount=\"%d\" Maximum=\"%d\" Minimum=\"%d\" Average=\"%d\">\n",
                i, tx * rx, stat_result[1], stat_result[2], stat_result[0]);
            for (j = 0; j < tx * rx; j++) {
    ...
    if (ts_test->item[GTP_SELFCAP_TEST]) {
        ...
        goodix_data_cal(ts_test->selfrawdata.data, tx + rx, stat_result);
        index += sprintf(&rbuf[index],
            "<DataContent No.=\"0\" DataCount=\"%d\" Maximum=\"%d\" Minimum=\"%d\" Average=\"%d\">\n",
            tx + rx, stat_result[1], stat_result[2], stat_result[0]);
        for (i = 0; i < tx + rx; i++) {
            index += sprintf(&rbuf[index], "%5d,", ts_test->selfrawdata.data[i]);
            if ((i + 1) % tx == 0)
                index += sprintf(&rbuf[index], "\n");
        }
        if ((tx + rx) % tx != 0)
    ...
    if (ts_test->item[GTP_STYLUS_RAW_TEST]) {
        ...
        for (i = 0; i < raw_data_cnt; i++) {
            ...
            index += sprintf(&rbuf[index],
                ...
                i, tx + rx, stat_result[1], stat_result[2], stat_result[0]);
            for (j = 0; j < tx + rx; j++) {
                index += sprintf(&rbuf[index], "%5d,", ts_test->stylusraw[i].data[j]);
                if ((j + 1) % tx == 0)
                    index += sprintf(&rbuf[index], "\n");
            }
            if ((tx + rx) % tx != 0)
...
static void goodix_save_test_result(struct goodix_ts_core *cd, bool is_brief)
{
    if (is_brief) {
        if (ts_test->item[GTP_CAP_TEST]) {
            ...
            index += sprintf(&rbuf[index], "%s\n", ts_test->result[GTP_CAP_TEST] ? "PASS" : "FAIL");
        }
        if (ts_test->item[GTP_DELTA_TEST]) {
            index += sprintf(&rbuf[index], "Delta test:\n");
            index += sprintf(&rbuf[index], "%s\n", ts_test->result[GTP_DELTA_TEST] ? "PASS" : "FAIL");
        }
        if (ts_test->item[GTP_SHORT_TEST]) {
            index += sprintf(&rbuf[index], "Short test:\n");
            index += sprintf(&rbuf[index], "%s\n", ts_test->result[GTP_SHORT_TEST] ? "PASS" : "FAIL");
        }
        if (ts_test->item[GTP_NOISE_TEST]) {
            index += sprintf(&rbuf[index], "Noise test:\n");
            index += sprintf(&rbuf[index], "%s\n", ts_test->result[GTP_NOISE_TEST] ? "PASS" : "FAIL");
        }
        if (ts_test->item[GTP_SELFCAP_TEST]) {
            index += sprintf(&rbuf[index], "Self test:\n");
            index += sprintf(&rbuf[index], "%s\n", ts_test->result[GTP_SELFCAP_TEST] ? "PASS" : "FAIL");
        }
    } else {
...
static int goodix_obtain_testlimits(struct goodix_ts_core *cd)
{
    ...
    sprintf(limit_file, "goodix_test_limits_%d.csv", cd->fw_version.sensor_id);
    ...
    if (ts_test->item[GTP_CAP_TEST]) {
        ...
        ret = parse_csvfile(temp_buf, firmware->size, CSV_TP_SPECIAL_RAW_DELTA,
                ts_test->deviation_limits, rx, tx);
        if (ret < 0) {
    ...
    if (ts_test->item[GTP_SELFCAP_TEST]) {
        ...
        ret = parse_csvfile(temp_buf, firmware->size, CSV_TP_SPECIAL_SELFRAW_MIN,
            ts_test->self_min_limits, 1, tx + rx);
        if (ret < 0) {
        ...
        ret = parse_csvfile(temp_buf, firmware->size, CSV_TP_SPECIAL_SELFRAW_MAX,
                ts_test->self_max_limits, 1, tx + rx);
        if (ret < 0) {
            ts_err("Failed get self_min_limits");
            goto exit_free;
        }
...
static void print_open_test_ng_data(struct goodix_ts_core *cd, int index, s16 *data)
{
    int tx = cd->ic_info.parm.drv_num;
    int rx = cd->ic_info.parm.sen_num;
    char output_buf[400];
    ...
    for (i = 0; i < rx; i++) {
        ...
        for (j = 0; j < tx; j++)
            cnt += sprintf(&output_buf[cnt], "%5d,", data[i * tx + j]);
        ts_info("%s", output_buf);
    }

    cd->hw_ops->read(cd, self_raw_addr, (unsigned char *)self_raw_data, (tx + rx) * 2);
    ts_info("self Tx Raw:");
    cnt = 0;
    memset(output_buf, 0, sizeof(output_buf));
    for (i = 0; i < tx; i++)
        cnt += sprintf(&output_buf[cnt], "%d,", self_raw_data[i]);
    ts_info("%s", output_buf);
    ts_info("self Rx Raw:");
    cnt = 0;
    memset(output_buf, 0, sizeof(output_buf));
...
static void print_self_compensation(struct goodix_ts_core *cd)
{
    ...
    if (cd->bus->ic_type == IC_TYPE_BERLIN_B) {
        ...
        for (i = 0; i < tx; i++) {
            ...
            cnt += sprintf(&output_buf[cnt], "%d,", val);
        }
        ts_info("%s", output_buf);
        cnt = 0;
        memset(output_buf, 0, sizeof(output_buf));
    ...
    for (i = 0; i < cfg_num; i++) {
        ...
        if (sub_cfg_index == 13) { // TX self cancel
            ...
            memset(output_buf, 0, sizeof(output_buf));
            for (j = 0; j < tx; j++) {
                val = le16_to_cpup((__le16 *)&cfg[2 + j * 4]) + le16_to_cpup((__le16 *)&cfg[4 + j * 4]);
                cnt += sprintf(&output_buf[cnt], "%d,", val);
            }
            ts_info("%s", output_buf);
...
static int goodix_open_test(struct goodix_ts_core *cd)
{
    ...
    for (i = 0; i < raw_data_cnt; i++) {
        ...
        for (j = 0; j < tx * rx; j++) {
            ...
            if (tmp_val > ts_test->max_limits[j] || tmp_val < ts_test->min_limits[j]) {
                err_cnt++;
                is_ng = true;
            }
        }
        if (is_ng)
            print_open_test_ng_data(cd, i, ts_test->rawdata[i].data);
    }
...
restart:
    ...
    for (j = 0; j < tx + rx; j++) {
        tmp_val = ts_test->selfrawdata.data[j];
         if (tmp_val > ts_test->self_max_limits[j] || tmp_val < ts_test->self_min_limits[j]) {
            // ts_err("self_rawdata[%d] out of range[%d %d]",
            //         tmp_val, ts_test->self_min_limits[j], ts_test->self_max_limits[j]);
    ...
    if (ts_test->result[GTP_SELFCAP_TEST] == TEST_NG && ng_rty > 0) {
        ng_rty--;
        cd->hw_ops->reset(cd, GOODIX_NORMAL_RESET_DELAY_MS);
        ts_info("self rawdata out of limit, retry:%d", SELF_RAWDATA_NG_RETRY - ng_rty);
        goto restart;
    }
...
static int goodix_noise_test(struct goodix_ts_core *cd)
{
    ...
    for (i = 0; i < noise_data_cnt; i++) {
		...
		if (retry < 0) {
			ts_err("noisedata is not ready val:0x%02x i:%d, exit", val, i);
            ret = -EAGAIN;
			goto exit;
		}
...
static int get_cap_data(struct goodix_ts_core *cd, uint8_t *type)
{
    ...
    if (strstr(type, CMD_GET_RAWDATA) ||
        ...
        for (i = 0; i < tx * rx; i++) {
            index += sprintf(&rbuf[index], "%5d,", *((s16 *)frame_buf + i));
            if ((i + 1) % tx == 0)
                index += sprintf(&rbuf[index], "\n");
        }
    } else {
...
static void goodix_read_config(struct goodix_ts_core *cd)
{
    ...
    cfg_ver = cfg_buf[34];
    index = sprintf(rbuf, "config_ver:0x%02X config_id:0x%04X\n", cfg_ver, cfg_id);
    kfree(cfg_buf);
}
...
static void goodix_set_custom_mode(struct goodix_ts_core *cd, u8 type, u8 val)
{
    ...
    if (type == PALM_FUNC) {
        ...
        if (val == 0) {
            index = sprintf(&rbuf[index], "set noise disable\n");
        } else if (val == 1) {
            index = sprintf(&rbuf[index], "set noise enable\n");
        } else if (val == 2) {
...
static void goodix_set_gesture_param(struct goodix_ts_core *cd, u8 type)
{
    ...
    if (retry < 0) {
        ts_err("failed to start write gesture param, status[%x]", status);
        goto exit;
    }
    if (type == GESTURE_STTW) {
        ts_info("STTW param:%*ph", gesture_param_st.length, gesture_param_st.buf);
        cd->hw_ops->write(cd, fw_buffer_addr, gesture_param_st.buf, sizeof(gesture_param_st));
    } else {
        ts_info("LPTW param:%*ph", gesture_param_lp.length, gesture_param_lp.buf);
        cd->hw_ops->write(cd, fw_buffer_addr, gesture_param_lp.buf, sizeof(gesture_param_lp));
    }
...
static void goodix_force_update(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    ...
    ret = goodix_do_fw_update(cd,
            UPDATE_MODE_BLOCK | UPDATE_MODE_FORCE | UPDATE_MODE_SRC_REQUEST);
    if (ret < 0)
...
static void goodix_run_self_open_test(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    int ret;
...
static void goodix_run_noise_test(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    int ret;
...
static void goodix_run_auto_noise_test(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 2) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_run_short_test(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    int ret;
...
static void goodix_get_package_id(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    int ret;
    u8 id;
...
static void goodix_get_version(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    ...
    index = sprintf(rbuf, "%s: 0x%02x%02x%02x%02x 0x%x\n", CMD_GET_VERSION,
                fw_ver.patch_vid[0], fw_ver.patch_vid[1],
                fw_ver.patch_vid[2], fw_ver.patch_vid[3],
...
static void goodix_get_self_rawdata(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    rbuf = malloc_proc_buffer(LARGE_SIZE);
    get_cap_data(cd, CMD_GET_SELF_RAWDATA);
}

static void goodix_get_self_diffdata(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    rbuf = malloc_proc_buffer(LARGE_SIZE);
    get_cap_data(cd, CMD_GET_SELF_DIFFDATA);
}

static void goodix_get_self_basedata(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    rbuf = malloc_proc_buffer(LARGE_SIZE);
    get_cap_data(cd, CMD_GET_SELF_BASEDATA);
}

static void goodix_set_double_tap_gesture(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 1) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_set_single_tap_gesture(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 1) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_set_long_press_gesture(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 1) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_set_st_param(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    ...
    ts_info("st_motion_tolerance:       %d", gesture_param_st.st_motion_tolerance);
...
static void goodix_set_lp_param(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    ...
    ts_info("lp_marginal_min_x:         %d", gesture_param_lp.lp_marginal_min_x);
    ts_info("lp_marginal_max_x:         %d", gesture_param_lp.lp_marginal_max_x);
    ts_info("lp_marginal_min_y:         %d", gesture_param_lp.lp_marginal_min_y);
    ts_info("lp_marginal_max_y:         %d", gesture_param_lp.lp_marginal_max_y);
    ts_info("lp_monitor_chan_min_tx:    %d", gesture_param_lp.lp_monitor_chan_min_tx);
    ts_info("lp_monitor_chan_max_tx:    %d", gesture_param_lp.lp_monitor_chan_max_tx);
    ts_info("lp_monitor_chan_min_rx:    %d", gesture_param_lp.lp_monitor_chan_min_rx);
    ts_info("lp_monitor_chan_max_rx:    %d", gesture_param_lp.lp_monitor_chan_max_rx);
    ts_info("lp_min_node_count:         %d", gesture_param_lp.lp_min_node_count);
    ts_info("lp_motion_tolerance_inner: %d", gesture_param_lp.lp_motion_tolerance_inner);
    ts_info("lp_motion_tolerance_outer: %d", gesture_param_lp.lp_motion_tolerance_outer);   
    goodix_set_gesture_param(cd, GESTURE_LPTW);
}

static void goodix_set_irq_enable(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 1) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_set_esd_enable(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 1) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_set_continue_mode(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static void goodix_get_channel_num(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    rbuf = malloc_proc_buffer(SHORT_SIZE);
...
static void goodix_set_sense_mode(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static void goodix_get_fw_status(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    ...
    index += sprintf(&rbuf[index], "coordfilter_status[%d] ", (val[0] >> 7) & 0x01);
    index += sprintf(&rbuf[index], "set_highsense_mode[%d] ", (val[0] >> 6) & 0x01);
    index += sprintf(&rbuf[index], "set_noise_mode[%d] ", (val[0] >> 4) & 0x03);
    index += sprintf(&rbuf[index], "set_water_mode[%d] ", (val[0] >> 3) & 0x01);
    index += sprintf(&rbuf[index], "set_grip_mode[%d] ", (val[0] >> 2) & 0x01);
    index += sprintf(&rbuf[index], "set_palm_mode[%d] ", (val[0] >> 1) & 0x01);
    index += sprintf(&rbuf[index], "set_heatmap_mode[%d]\n", (val[0] >> 0) & 0x01);
    index += sprintf(&rbuf[index], "fw_ns/hs[%d]", (val[1] >> 0) & 0x01);
    index += sprintf(&rbuf[index], "wireless mode[%d]", (val[1] >> 1) & 0x01);
...
static void goodix_set_highsense_mode(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static void goodix_set_noise_mode(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 1) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_set_water_mode(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    if (bufsz != 1) {
        ts_err("invalid cmd size:%d", bufsz);
        return;
...
static void goodix_get_self_compensation(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    u8 *cfg;
    u8 *cfg_buf;
    int len;
...
static void goodix_set_report_rate(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static void goodix_get_dump_log(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    ...
    for (i = 0; i < DUMP_AREA1_LEN; i++) {
        index += sprintf(&rbuf[index], "%02x,", debug_buf[i]);
        if ((i + 1) % 32 == 0)
            index += sprintf(&rbuf[index], "\n");
    }
...
static void goodix_set_freq_index(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static void goodix_disable_coor_filter(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static void goodix_get_im_rawdata(struct goodix_ts_core *cd, int *buf, int bufsz)
{
...
static void goodix_set_hsync_speed(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static void goodix_set_wireless_charge(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
    ...
    rbuf = malloc_proc_buffer(SHORT_SIZE);
    index = sprintf(rbuf, "%s wireless mode\n", (buf[0] == 0) ? "exit" : "enter");
    temp_cmd.len = 5;
    temp_cmd.cmd = 0xAF;
    temp_cmd.data[0] = (buf[0] == 0) ? 0 : 1;
    cd->hw_ops->send_cmd(cd, &temp_cmd);
...
static void goodix_set_continue_heatmap(struct goodix_ts_core *cd, int *buf, int bufsz)
{
    struct goodix_ts_cmd temp_cmd;
...
static struct cmd_handler cmd_handler_list[] = {
    ...
    {CMD_AUTO_TEST, goodix_run_auto_test},
    {CMD_STYLUS_RAW_TEST, NULL},
    {CMD_STYLUS_OSC_TEST, NULL},
    {CMD_GET_STYLUS_DATA, NULL},
    ...
    {CMD_GET_TX_FREQ, goodix_get_tx_freq},
    {CMD_RESET, goodix_hw_reset},
...
int driver_test_proc_init(struct goodix_ts_core *core_data)
{
	...
	proc_entry = proc_create_data("driver_test",
			0660, core_data->proc_dir_entry, &driver_test_ops, core_data);
	if (!proc_entry) {
        ts_err("failed to create proc driver_test");
        return -ENODEV;
    }

	proc_entry = proc_create("cmd_list",
			0440, core_data->proc_dir_entry, &cmd_list_ops);
    if (!proc_entry) {
