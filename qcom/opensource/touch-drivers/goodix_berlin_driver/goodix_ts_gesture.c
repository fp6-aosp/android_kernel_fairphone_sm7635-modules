...
int goodix_ts_report_gesture(struct goodix_ts_core *cd, struct goodix_ts_event *event)
{
	...
	coor_x = le16_to_cpup((__le16 *)event->gesture_data);
	coor_y = le16_to_cpup((__le16 *)(event->gesture_data + 2));
	coor_size = le16_to_cpup((__le16 *)(event->gesture_data + 4));
	coor_press = event->gesture_data[6];
	major = le16_to_cpup((__le16 *)(event->gesture_data + 7));
	minor = le16_to_cpup((__le16 *)(event->gesture_data + 9));
	orientation = (s8)event->gesture_data[11];
	...
	case GOODIX_GESTURE_SINGLE_TAP:
		if (cd->gesture_type & GESTURE_SINGLE_TAP) {
			...
			ts_debug("fodx:%d fody:%d size:%d press:%d maj:%d min:%d ori:%d",
					coor_x, coor_y, coor_size, coor_press, major, minor, orientation);
			...
			input_report_abs(cd->input_dev, ABS_MT_POSITION_X, coor_x);
			input_report_abs(cd->input_dev, ABS_MT_POSITION_Y, coor_y);
			input_report_abs(cd->input_dev, ABS_MT_PRESSURE, coor_press);
			input_report_abs(cd->input_dev, ABS_MT_TOUCH_MAJOR, major);
			input_report_abs(cd->input_dev, ABS_MT_TOUCH_MINOR, minor);
