...
static void goodix_opencircuit_test(struct goodix_ts_test *ts_test)
{
	...
	while (retry--) {
		ret = ts_test_read(ts_test, PRODUCT_TEST_ADDR, buf, sizeof(buf));
		if (ret < 0) {
	...
	if (buf[2] == 0) {
		...
		for (i = 0; i < DRV_CHAN_BYTES; i++) {
			if (buf[i + 3])
				ts_info("DRV[%d~%d] open circuit, ret=0x%X", i * 8, i * 8 + 7, buf[i + 3]);
		}
		...
		struct ts_rawdata_info *info)
{
	...
	cnt = snprintf(info->result, TS_RAWDATA_RESULT_MAX - 1, "open_test-[%s] ",
			(ts_test->test_result[GTP_OPEN_TEST] == GTP_TEST_PASS) ? "PASS" : "FAIL");
	snprintf(&info->result[cnt], TS_RAWDATA_RESULT_MAX - 1, "short_test-[%s]\n",
			(ts_test->test_result[GTP_SHORT_TEST] == GTP_TEST_PASS) ? "PASS" : "FAIL");
