		...
		struct firmware_data *fw_data)
{
	...
	int header_size =
		cd->update_ctrl.update_info->header_size;
	int r = 0;
	u32 cfg_flash_addr;
	...
	for (i = 0; i < fw_summary->subsys_num; i++) {
		...
		if (cd->board_data.use_one_binary) {
			if (fw_summary->subsys[i].flash_addr == cfg_flash_addr) {
				cd->update_ctrl.cfg_id = le32_to_cpup((__le32 *)&fw_summary->subsys[i].data[30]);
				ts_info("Firmware config id:0x%x", cd->update_ctrl.cfg_id);
			}
		}
	}
...
static int goodix_read_pid_from_flash(struct goodix_ts_core *cd, u8 *pid)
{
	u8 buf[8];
	int ret;
...
int goodix_fw_update_init(struct goodix_ts_core *core_data)
{
	...
	if (core_data->bus->ic_type == IC_TYPE_BERLIN_D) {
		ret = goodix_read_pid_from_flash(core_data, pid);
		if (ret == 0) {
			//TODO: select firmware.bin based on PID
			memset(core_data->update_ctrl.fw_name, 0, sizeof(core_data->update_ctrl.fw_name));
			if (!memcmp(pid, "9916R", 5)) {
					strlcpy(core_data->update_ctrl.fw_name, "goodix_firmware.bin",
						sizeof("goodix_firmware.bin"));
			} else if (!memcmp(pid, "9916P", 5)) {
					strlcpy(core_data->update_ctrl.fw_name, "goodix_firmware.bin",
						sizeof("goodix_firmware.bin"));
			}
		}
