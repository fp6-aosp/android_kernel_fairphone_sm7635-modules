...
#endif
{
	...
	const char *dvdd_supply_str = "pa-dvdd-supply";
	const char *dvdd_supply_str1 = "pa-dvdd";
		
	aw_pr_info("enter addr=0x%x", i2c->addr);

//DVDD VREG_L10B_1P8
	if (of_parse_phandle(np, dvdd_supply_str, 0)) {
		dvdd_supply = devm_regulator_get(&i2c->dev, dvdd_supply_str1);
		if (IS_ERR(dvdd_supply)) {
			ret = PTR_ERR(dvdd_supply);
			aw_dev_err(&i2c->dev,"%s:Failed to get DVDD VREG_L10B_1P8 %d\n", __func__, ret);
			return ret;
		}
		ret = regulator_enable(dvdd_supply);
		if (ret) {
			aw_dev_err(&i2c->dev,"%s: regulator DVDD VREG_L10B_1P8 enable failed, err = %d\n", __func__, ret);
			return ret;
		} else {
			aw_pr_info(" DVDD VREG_L10B_1P8 enable success");
		}
	}
//end
...
//DVDD VREG_L10B_1P8
	if (!dvdd_supply) {
		aw_pr_info("%s: DVDD VREG_L10B_1P8 regulator not provided in dtsi\n", __func__);
	} else {
		ret = regulator_disable(dvdd_supply);
		if (ret) {
			aw_pr_info("%s: DVDD VREG_L10B_1P8 regulator disable failed, err = %d\n", __func__, ret);
		} else {
			aw_pr_info(" DVDD VREG_L10B_1P8 disable success");
		}
	}
//end
